...
- `setContract({ maxInteractionLatencyMs, protectedElement, active }, caller)` turns the promise on or off. The ceiling is an integer from 50 to 200 ms. Its protected element may be declared `neverShed` or already protected through `protectElement`. A ladder-step target is refused with a receipt that tells the caller to pin or remove the step instead. `active: false` is contrast mode: it restores all shed work, preserves receipts, prevents automatic shedding, and writes an explicit override receipt for the human's full-fidelity choice.
